...
\epubtkImage{}{%
\begin{figure}[hptb]
  \centerline{\includegraphics[width=220pt]{LIGOsens}}
...
$10^{-18}$ for millisecond pulses {\color{red} ??} -- such that the technology could be
...
decommissioning began in preparation for the installation of Advanced LIGO.
...
The operation and commissioning of these detectors is a frequently evolving 
...
CLIOweb} designed to test technologies for a future large scale Japanese detector
...
through S6 \cite{LIGOcurves} ({\color{red} Note that the S6 curve is 
...
2010, at which point decommissioning started for the full upgrade to AdvLIGO. 
...
power) and $\approx 20$~W at night, leading to angle averaged inspiral ranges 
from $\approx 10--20$\,Mpc.
...
S6 in an overnight and weekend mode, along side a commissioning schedule, and is 
...
time science runs WSR1, WSR10, VSR1 and VSR2 \cite{Virgocurves} - {\color{red} must check
that we can use these curves as they're actually in a password protected area}.}
...
The main upgrade is also to the lasers to increase their power from 10 to 25~W at
...
of $6\times{-22}\,{\rm Hz}^{-1/2}$ to $2\times{-20}\,{\rm Hz}^{-1/2}$, and a
90\% confidence event rate for signal between 64--2048~Hz of less than two 
per year.
...
\cite{Abadie:2010f} giving 90\% confidence upper rates of $8.7\times{-3}$ per year per $L_{10}$, 
$2.2\times{-3}$ per year per $L_{10}$, and $4.4\times{-4}$ per year per $L_{10}$.
...
During S2 a particularly bright $\gamma$-ray burst event (GRB030329) that occurred 
...
limit that the source distance must be at a distance greater than 3.5\,Mpc. 
...
signals \cite{Abbott:2010}. No evidence for a gravitational wave signal 
...
be highly magnetised neutron stars called {\it magnetars}. During the first year of 
...
and for $f$-mode ring-downs (at 1090 Hz), of $3.0\times{44}$~erg and $1.4\times{47}$~erg 
...
1--3~kHz this provided upper limits on the peak strain of $0.6-1.4\times{-20}$
...
wave data for external triggers, and instead supply gravitational wave burst 
...
J0537$-$6910. Of the other pulsars the best (lowest) upper limit on 
gravitational wave amplitude is $2.3\times{–26}$ for J1603$-$7202 and 
our best (lowest) limit on the inferred pulsar ellipticity is $7.0\times{–8}$ 
for J2124$-$3358.
...
band of $(0.7-1.2)\times{–24}$ and $(0.4-4)\times{–4}$ respectively. These results
beat indirect limits on the emission based on energy conservation arguments 
(similar, but not the same as the spin-down limits) and were also for the first cast
as limits on the $r$-mode amplitude.

\subsubsection{Stochastic sources}
There are searches for a cosmological, or astrophysical, background of gravitational 
waves, which would show up as a coherent stochastic noise source between detectors. 
...
will be used. Initial/Enhanced LIGO was shut down to begin the installation of these
upgrades on 20 October 2010. ({\bf Add the current
...
in a cube of electrodes designed to capacitively sense their position and designed
