...
\epubtkImage{}{%
\begin{figure}[hptb]
  \centerline{\includegraphics[scale=0.4]{LIGOsens}}
  \caption{\it Proposed sensitivity for the LIGO\protect~1 and planned
    LIGO\protect~2 detectors.}
...
\begin{figure}[hptb]
  ...
  \caption{\it Schematic view of one suspension system for use in the
    GEO\protect~600 interferometer.}
  \label{figure:GEOsuspension}
\end{figure}}
...
\section{Long Baseline Detectors in Operation}
...
\S\ref{subsections:runs} for a review of recent science data runs and results.)

The American LIGO project \cite{LIGOweb} comprises the building of two detector
...
respectively.) One half length, 2~km, interferometer is also contained inside
...
events within the local group of galaxies, but also is a test bed for developing
...
Memorandum of Understanding, under the auspices of the LIGO Scientific
...
detectors. The Virgo collaboration have also signed a Memorandum of
Understanding with the LSC, which has allowed data sharing since May 2006.
...
\subsubsection{LIGO}
The first LIGO detector to achieve lock, i.e. with the interferometers locked on
a dark fringe with light resonating throughout the cavity, was H2 in late 2000. 
By early 2002 all three detectors had full lock, and they have since undergone
many periods of commissioning and science data taking. Over the period between
mid-2001 to mid 2002 the commissioning process improved the detector's peak
sensitivities by several orders of magnitude, with the LLO 4k going from
...
\cite{Abbott:2005a} for a more thourough description of the detector
improvements made for S2.) These improved the sensitivities by about an order of
...
coincidence time when all three detectors were in lock all of 22\% of the run.
...
Since the end of S5 the LIGO H2 detector and GEO~600 have been kept operational
while possible in an evening and weekend mode called Astrowatch. This
observing mode is expected to last until early 2009.
...
instruments. The autoalignment system in GEO~600 has since meant that it has
...
taken offline for more commissioning work. In the period between S3I and II 
various sources of noise lock loss were diagnosed and mitigated, including
noise form a servo in the signal recycling cavity and electronic noise on a
...
conincidence with LIGO, with a $\sim$97\% duty factor. It used the same optical
...
power-recycling, end by the end of 2004 lock with power recycling was achieved.
...
begin on 18 May 2007 and ended with the end of S5 on 1 October 2007. During VSR1
...
\cite{Hereld:1984}, and a on the rate of neutron star binary
...
S4 run. At the time of writing much of the data from the S5 run have yet to be
fully analysed, with the majority of results expected over the next year. A
...
are commom features in the data, so to veto these events from true
...
detectors data stream with respect the the others. In this way you should only
...
event rate of 1.6 events per day was able to be set. The search was typically
...
the best sensitivities astrophysically gave order of magnitude estimates of on
...
AURIGA bar detector and this data has been analysed \cite{Baggio:2008}. This was
a much smaller bandwidth search, due to bars sensitivity around its resonant
mode, and looked for short bursts of less than 20~ms within the 850-950~Hz band.
This had comparable sensitivity to the LIGO-only S2 search and produced a 90\%
...
signals with just below $h_{\rm rss}\sim10^{-20}$ and set a 90\% confidence rate
...
efficiency to signals of sin-Gaussian nature (at the most sensitive frequency of
...
cluster ($\sim16$\,Mpc) for an energy release of $0.25$\,M$_{\odot}c^2$. Also
see \cite{Abbott:2007b} for a comparison of previous burst searches. There was
...
and used 257 hours of quadrupole coincidence between the detectors and saw no
...
INTEGRAL and Swift as distributed by the BRB Coordinate Network \cite{GCN}. The
search looked and a 180 second window around the burst peak time (120 seconds
...
sensitive to sine-Gaussian signals out to 10s of Mpc for an energy release of
order a solar mass.
...
by Swift on 15 September 2005 was Virgo data used to search for an unmodelled
burst in a window of 180 seconds around (120 s before and 60 s after) the GRB
peak time \cite{Acernese:2008a}. The search produced strain upper limit of order
$10^{-20}$ in the frequency range 200--1500 Hz, but was mainly used  a test-bed
...
has been to looked from gravitational waves associated with flares from known
...
equation to give, which in turn can give the phase evolution of the orbit, and
more recently have started to include numerical relativity simulations of the
merger stage. The current best estimates for binary neutron star merger rates,
based on the known population of binary neutron star systems, give a 95\%
confidence range between 10-170$\times10^{-6}$ per year per $L_{10}$
...
The majority of inspirals searches make use of matched filtering in which a
...
for compact object coalescence's with component masses between 1--3\,M$_{\odot}$
...
the S1 and S2 LIGO search, but was then changed to the $L_10$ unit, although
...
$\sim1.5$\,Mp, which spanned the Local Group of galaxies, and gave a 90\% event
...
designed specifically for systems with spinning components. It search for
systems with component masses in the range 1--20\,M$_{ \odot}$ and analyses 167
...
thourough discussion of this argument can be found in \cite{Knispel:2008}.)
...
Doppler demodulated to take into account the signals signals shifting frequency
...
consisted of Gaussian noise alone. As selection of the searches are summarised
...
higher than the limit that can be set by equating the stars total rate of loss
...
pulsar closest to it's inferred spin-down limit was that Crab pulsar
...
0--$-1.1\times10^{-9}$\,Hz\,s$^{-1}$ \cite{Abbott:2005g}. This gave a lowest
...
by participants in the project and ran as a background process or screensaver.
The search band spanned the range from 50--1500.5~Hz. The search saw now
plausible gravitational wave candidates and result is described at
\cite{eathS3}, but it was not used to produce and upper limit.

In the known pulsar search the number of sources search for using the combined
...
50--1500~Hz \cite{Abbott:2008f}. The range of spin-down's $\dot{f}$ was chosen
by using a minimum spin-down ago $\tau$ and having $-f/\tau < \dot{f} <
0.1f/\tau$, with $\tau = 1000$ years for signals below 300~Hz and $\tau = 10000$
...
This produce an 90\% root mean squared upper limit on gravitational wave strain
...
inertia). These beat Crab pulsar's spin-down limit by 4 to 5 times and can
...
generation detectors, which could provide a the premier gravitational wave
...
be found here \cite{Whitcomb:2008}.

\subsubsection{Enhanced LIGO, Virgo+ and GEO-HF}
...
by a factor of two to three times for the following run. This project has been
called Enhanced LIGO \cite{EnhancedLIGO} and is well underway for completion in
early 2009. Enhanced LIGO involves the direct implementation of technologies
and techniques designed for the later upgrade to Advanced LIGO (AdLIGO - see
...
several of the optical components need to be replaced. These upgrades are only
being carried out on the 4km H1 and L1 detectors due to the H2 detector being
left in Astrowatch mode during the commissioning period. It is expected that
Enhanced LIGO will carry out an approximately two year science run from spring
2009 before being taken off-line for the full upgrade to AdLIGO.
...
\cite{Acernese:2008b, AdVwhitepaper}. The aim is to have comparable sensitivity
to Enhanced LIGO and have these upgrades ready by mid-2009.
...
that a there is realistic detection rate of binary neutron star coalescences of
around 40${\rm yr}^{-1}$ \cite{Kopparapu:2008}. These technological issues
...
readout, would need a separate review article themself, but we shall very
...
10. There currently exists a technology demonstator called Cryogenic Laser
...
currently underway into a variety of these issues.

Investigations into the interferometric configuration have already been studied
(see \cite{Freise:2008, Hild:2008}), which suggest a triple interferometer
system made up from an equilateral triangle.
...
Perhaps the most interesting gravitational wave signals -- those resulting from
the formation and coalescence of black holes in the range $10^3$ to $10^6$ solar
masses --  will lie in the region of $10^{-4}$~Hz to $10^{-1}$~Hz, and a
detector whose strain sensitivity is approximately $10^{-23}$ over relevant
timescales is required to search for these. The most promising way of looking
for such signals is to fly a laser interferometer in space, i.e. to launch a
number of drag free space craft into orbit and to compare the distances between
test masses in these craft using laser interferometry. LISA -- Laser
Interferometer Space Antenna (see for example~\cite{LISAsymposium, NASAweb,
ESAweb}) -- is being proposed by an American/European team; it consists of an
array of three drag free spacecraft at the vertices of an equilateral triangle
of length of side $5 \times 10^6$~km. This cluster is placed in an Earth-like
orbit at a distance of 1~AU from the Sun, and 20 degrees behind the Earth. Proof
masses inside the spacecraft (two in each spacecraft) form the end points of
three separate but not independent interferometers. Each single two-arm
...
wavelength of 1.064 microns, laser powers of $\simeq$~1~W being available from
...
to achieve its design performance, adjacent arm lengths have to be sensed to an
accuracy of  better than $30 {\rm \ pm}(\rm Hz)^{-1/2}$. Because of the long
...
area for reception of the beam.  Given that the available laser power in each
arm is of the order of 1~W, and that arguments similar to those already
discussed for ground based detectors can be made, photoelectron shot noise
considerations suggest that the diameters of the transmitting and receiving
mirrors on the space craft need to be $\simeq$~30~cm.  
...
gravitational waves may be dominated by frequency noise. For the $5 \times 10^9$
m long arms of LISA, a difference in arm length of $10^8$~m is likely. Then for
...
stabilisation has to be provided. The primary method of stabilisation is to lock
the frequency of one laser in the system on to a Fabry-Perot cavity mounted on
...
Hz}(\rm Hz)^{-1/2}$. This level of laser frequency noise is clearly much worse
than the required $6 \times 10^{-6}  {\rm \ Hz}(\rm Hz)^{-1/2}$ and a further
correction scheme is needed. Further frequency correction is provided by
comparing the phase of the light returning in each arm with the phase of the
transmitted light. The phase difference, measured over the time of flight in the
arm, allows an estimate of laser frequency noise to be made~\cite{Shoemaker,
Faller2, Giampieri}. For the arm of length $L$
%
\begin{equation}
  \delta \phi = (4 \pi /c) L \delta \nu,
  \label{equation:fnoiselisa}
\end{equation}
%
and thus if the spectral density $\delta \phi$ is measured, the spectral density
$\delta \nu$ can be estimated.  This estimate can then be used to correct the
signal obtained by subtracting the phase difference measurements in two adjacent
arms, allowing the search for gravitational waves to be carried out.  This
correction is made easier if each arm length is known to a few km and the
difference in arm length is known to a few tens of metres.  These quantities
should be available from radar and optical ranging measurements.  If however
they are not well enough known then they can be found by searching through a
range of possible values to minimise the effect of frequency noise on the
...
to~\cite{Houghetal} for a discussion of some of these.

%LISA has been adopted by ESA as a Cornerstone project in their post Horizon
%2000 programme and the possibility of it being a joint ESA/NASA collaborative
%mission is being enthusiastically addressed at present.
